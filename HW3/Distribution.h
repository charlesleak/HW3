...
class uniformShell_distribution : public distribution< point > {
  private:
    double x0, y0, z0;
    uniform_distribution dist_phi, dist_mu, dist_r;
  public:
    uniformShell_distribution( std::string label, double x, double y, double z, double R, double r ) 
      : distribution(label), x0(x), y0(y), z0(z), dist_phi( "uniformShell phi", 0.0, 2.0 * std::acos(-1.0) ),
      dist_mu( "uniformShell mu", -1.0, 1.0 ), dist_r( "uniformShell r", r, R ) {};
    ~uniformShell_distribution() {};
    point sample();
};

class uniformZCylinder_distribution : public distribution< point > { // cylinder is a circle extruded in -z by dz
  private:
    double x0, y0;
    uniform_distribution dist_phi, dist_r, dist_z;
  public:
    uniformZCylinder_distribution( std::string label, double x, double y, double z, double R, double dz ) 
      : distribution(label), x0(x), y0(y), dist_phi( "uniformZCylinder phi", 0.0, 2.0 * std::acos(-1.0) ),
      dist_r( "uniformZCylinder r", 0, R ), dist_z( "uniformZCylinder z", z, z - dz) {};
    ~uniformZCylinder_distribution() {};
    point sample();
};

class uniformXCylinder_distribution : public distribution< point > { // cylinder is a circle extruded in -x by dx
  private:
    double y0, z0;
    uniform_distribution dist_phi, dist_r, dist_x;
  public:
    uniformXCylinder_distribution( std::string label, double x, double y, double z, double R, double dx ) 
      : distribution(label), y0(y), z0(z), dist_phi( "uniformXCylinder phi", 0.0, 2.0 * std::acos(-1.0) ),
      dist_r( "uniformXCylinder r", 0, R ), dist_x( "uniformXCylinder x", x, x - dx) {};
    ~uniformXCylinder_distribution() {};
    point sample();
};
