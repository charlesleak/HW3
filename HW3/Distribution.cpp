...
point uniformShell_distribution::sample() {
  double mu = dist_mu.sample();
  double phi = dist_phi.sample();
  double r = dist_r.sample();
  double x = r * std::sqrt( 1.0 - mu * mu ) * cos( phi );
  double y = r * std::sqrt( 1.0 - mu * mu ) * sin( phi );
  double z = r * mu;
  return point( x + x0, y + y0, z + z0 );
}

point uniformZCylinder_distribution::sample() {
  double phi = dist_phi.sample();
  double r = dist_r.sample();
  double z = dist_z.sample();
  double x = r * cos( phi );
  double y = r * sin( phi );
  return point( x + x0, y + y0, z );
}

point uniformXCylinder_distribution::sample() {
  double phi = dist_phi.sample();
  double r = dist_r.sample();
  double x = dist_x.sample();
  double y = r * cos( phi );
  double z = r * sin( phi );
  return point( x, y + y0, z + z0 );
}
