...
class material {
  private:
    ...
    double micro_xs( particle* p );    // returns micro xs of material for use by macro_xs
  public:
    ...
    double macro_xs( particle* p );                // return the material's macro xs
    std::shared_ptr< nuclide > sample_nuclide( particle* p );         // sample nuclide based on cross sections and atom fractions
    std::string sample_collision( particle* p, std::stack<particle>* bank ); // samples nuclide, samples reaction from nuclide, calls reaction's sample method
};
