...
class surface {
  ...
  public:
    ...
    virtual void scoreEstimators( particle* p, double s ) final {                         // uses estimator's score method
      for ( auto e : surface_estimators ) { e->score( p, s ); }
    }

    virtual void crossSurface( particle* p, double s ) final {                            // scores estimators, reflects, nudges particle
      // score estimators
      for ( auto e : surface_estimators ) { e->score( p, s ); }
...
class conex : public surface { // cone pointing in +x with point at x,y,z = 0
  private:
    double slope;
  public:
    conex( std::string label, double p1 ) :   // constructor takes name and cone equation
    surface(label), slope(p1) {};
    ~conex() {};             // destructor
