...
double conex::distance( ray r ) { //http://www.wolframalpha.com/input/?i=(y%2Bj*s)%5E2+%2B+(z%2Bk*s)%5E2+-+(x%2Bi*s)%5E2*d%5E2+%3D+0+solve+for+s
  double x = r.pos.x;
  double y = r.pos.y;
  double z = r.pos.z;
  double i = r.dir.x;
  ...
  double term1 = slope * slope + j * j + k * k;

  if ( std::fabs( term1 ) > 100.0 * std::numeric_limits<double>::epsilon() ) {
    double term2 = 0.5 * std::sqrt( -4.0 * ( slope*slope + j*j + k*k) * ( -1.0 * slope*slope * x*x + y*y + z*z ) + 4.0 * std::pow( -1.0 * i * slope*slope * x + j * y + k * z, 2 ) );
    double term3 = i * slope*slope * x - j * y - k * z;
    double dist = ( term2 + term3 ) / term1;
    if ( dist > 0.0 ) { return dist; }
    else { return std::numeric_limits<double>::max(); }
  }
  else {
