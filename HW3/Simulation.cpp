...
simulation::simulation( std::string input_file_name ) {
  ...
  while ( set_distributions < num_distributions ) {
	...
	else if ( type == "uniformShell" ) {
	  double x = d.attribute("x").as_double();
	  double y = d.attribute("y").as_double(); 
	  double z = d.attribute("z").as_double(); 
	  double R = d.attribute("R").as_double(); 
	  double r = d.attribute("r").as_double(); 
	  Dist = std::make_shared< uniformShell_distribution > ( name, x, y, z, R, r );
	}
	else if ( type == "uniformZCylinder" ) {
	  double x = d.attribute("x").as_double();
	  double y = d.attribute("y").as_double(); 
	  double z = d.attribute("z").as_double(); 
	  double R = d.attribute("R").as_double(); 
	  double dz = d.attribute("dz").as_double(); 
	  Dist = std::make_shared< uniformShell_distribution > ( name, x, y, z, R, dz );
	}
	else if ( type == "uniformXCylinder" ) {
	  double x = d.attribute("x").as_double();
	  double y = d.attribute("y").as_double(); 
	  double z = d.attribute("z").as_double(); 
	  double R = d.attribute("R").as_double(); 
	  double dx = d.attribute("dx").as_double(); 
	  Dist = std::make_shared< uniformShell_distribution > ( name, x, y, z, R, dx );
	}
  ...
  for ( auto n : input_nuclides ) {
    ...
    double A = n.attribute("atomicMass").as_double();

    std::shared_ptr< nuclide > Nuc = std::make_shared< nuclide > ( n.attribute("name").value(), A ); // if A is provided, use it in the constructor
    ...
    for ( auto r : n.children() ) {
      ...
      double k = r.attribute("oneOverVFactor").as_double();
      if ( rxn_type == "capture" ) {
        Nuc->addReaction( std::make_shared< capture_reaction > ( xs, k ) );
      }
        ...
        if ( scatterDist ) {
          Nuc->addReaction( std::make_shared< scatter_reaction > ( xs, k, scatterDist ) );
        }
        else {
        ...
        if ( multDist ) {
          Nuc->addReaction( std::make_shared< fission_reaction > ( xs, k, multDist ) );
        }
        else {
  ...
  for ( auto s : input_surfaces ) {
    ...
    if ( type == "plane" ) {
      ...
      std::string name = s.attribute("name").value();
      double slope = s.attribute("slope").as_double();
      S = std::make_shared< conex > ( name, slope );
    }
    else {
  ...
  for ( auto e : input_estimators ) {
    ...
    else if ( type == "trackLengthTimed" ) {
      Est = std::make_shared< track_length_timed_estimator > ( name );
      // get the cells
      for ( auto s : e.children() ) {
        if ( (std::string) s.name() == "cell" ) {
          std::string name = s.attribute("name").value();
